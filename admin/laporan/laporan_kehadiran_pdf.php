...
if (@$_POST['id_jenispelatihan']) 
{
	...
	class FPDF_AutoWrapTable extends FPDF {
		...
		public function rptDetailData () {
			...
			$this->SetFont("", "B", 18);
			$this->SetX(225);$this->MultiCell(500, 20, 'Dinas Tenaga Kerja, Perindustrian, Koperasi, Usaha Kecil dan Menengah Kabupaten Kudus',0, 'C');
			$this->Cell(0, 20, " ", "B");
			$this->Ln(30);
			$this->SetFont("", "B", 12);
			$this->SetX($left); $this->Cell(0, 10, 'Laporan Data Kehadiran Pelatihan '.explode('|', $_POST['id_jenispelatihan'])['1'], 0, 1,'C');
			$this->Ln(10);
			...
			if ($this->data['tahun']) 
			{
				...
				$this->SetX($left += 20); $this->Cell(50, $h*3, 'ID Peserta', 1, 0, 'C',true);
				$this->SetX($left += 50); $this->Cell(90, $h*3, 'No KTP', 1, 0, 'C',true);
				$this->SetX($left += 90); $this->Cell(150, $h*3, 'Nama Peserta', 1, 0, 'C',true);
				...
				{
					$this->SetX($left += $last_left); $this->Cell($size * $value['w'], $h, $value['n'], 1, 0, 'C',true);
				...
				{
					$left = 10;
					$this->SetX($left += 20); $this->Cell(50, $h, $key, 1, 0, 'C',true);
					$this->SetX($left += 50); $this->Cell(90, $h, $value['no_ktp'], 1, 0, 'L',true);
					$this->SetX($left += 90); $this->Cell(150, $h, $value['nama_peserta'], 1, 0, 'L',true);
					$last_left = $last_left_set;
					$left = 170;
					foreach ($value['kehadiran'] as $key1 => $value1) 
					{
						$this->SetX($left += $last_left); $this->Cell($size * 1, $h, $value1, 1, 0, 'C',true);
						$last_left = $size * 1;
					}
					$this->Ln(25);
				}
...
$i = 0;
$result = Array();
while ($datane = mysql_fetch_array($q_kepala)) {
  $result[$i++] = $datane[0];
}
		    ...
		    $this->Ln(10);
		    $this->SetFont("Arial", "B", 9);
			$this->Cell(0, 10, 'Kudus, '.$tgl, 0, 1,'R');
			$this->Ln(1);
			$this->Cell(0, 9, 'Kepala Dinas                                                                                                         Kepala Bidang Perindustrian', 0, 1,'C');
			$this->Ln(60);
			// pr($GLOBALS);
			$this->SetX(0); $this->Cell(0, 10, '( '.$result[0].' )                                                                                  ( '.$result[1].' )', 0, 1,'C');
	...
	include "../../koneksi.php";
	$id_jenispelatihan = explode('|', $_POST['id_jenispelatihan'])['0'];
	#ambil data dari DB dan masukkan ke array
	$tgl_kehadiran = array();
	$s_pre = mysql_query("SELECT date_format(tgl,'%d') as hari, date_format(tgl,'%M') as bulan, date_format(tgl,'%Y') as tahun, GROUP_concat(id_peserta) as peserta FROM `tb_kehadiran` natural join tb_pelatihan natural join tb_pendaftaran WHERE tb_pendaftaran.status='diterima' and id_jenispelatihan='".addslashes(@$id_jenispelatihan)."' GROUP by tgl") or die(mysql_error());
	...
	$s_pendamping = mysql_query("SELECT * FROM tb_pendaftaran natural join tb_pelatihan where id_jenispelatihan = '".addslashes(@$id_jenispelatihan)."' and status='diterima'") or die(mysql_error());
	while ($data = mysql_fetch_array($s_pendamping)) 
	{
	  ...
	  foreach ($tgl_kehadiran as $key => $value) 
	  {
		...
		$r_tb_header['data'][$data['id_peserta']]['kehadiran'] = $is_masuk;
	}
	// pr($r_tb_header);
	// exit();
	$data = $r_tb_header;
