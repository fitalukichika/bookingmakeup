<?php  
	...
	class FPDF_AutoWrapTable extends FPDF {
		...
		public function rptDetailData () {
			...
			$this->Ln(30);
			$this->SetFont("", "B", 12);
			$this->SetX($left); $this->Cell(0, 10, 'Laporan Data Peserta ', 0, 1,'C');
			$this->Ln(10);
			...
			$this->SetX($left += 20); $this->Cell(60, $h, 'ID Peserta', 1, 0, 'C',true);
			$this->SetX($left += 60); $this->Cell(100, $h, 'Nama', 1, 0, 'C',true);
			$this->SetX($left += 100); $this->Cell(200, $h, 'Alamat', 1, 0, 'C',true);
			$this->SetX($left += 200); $this->Cell(50, $h, 'Jekel', 1, 0, 'C',true);
			$this->SetX($left += 50); $this->Cell(100, $h, 'Telp', 1, 0, 'C',true);
			$this->SetX($left += 100); $this->Cell(150, $h, 'Nama Perusahaan', 1, 0, 'C',true);
			$this->SetX($left += 150); $this->Cell(150, $h, 'Nama Pelatihan', 1, 0, 'C',true);
			$this->SetX($left += 150); $this->Cell(50, $h, 'Periode', 1, 0, 'C',true);
			$this->Ln(25);
		...
		public function printPDF () {
			...
			$q_kepala = mysql_query("SELECT nama from tb_user WHERE status in ('kabid','kadin')") or die(mysql_error());
$i = 0;
$result = Array();
