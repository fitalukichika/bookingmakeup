...
$query = mysql_query($tampilkan) or die(mysql_error());
...
if (isset($_POST['simpan'])) {
        ...
        if ($foto_diri == "" && $tdi == "") {
             $simpan = mysql_query("UPDATE tb_pendaftaran SET id_peserta='$id_peserta', no_ktp='$no_ktp', nama_peserta='$nama_peserta', alamat='$alamat', no_hp='$no_hp', nama_perusahaan='$nama_perusahaan', produk='$produk', username='$username', password='$password', status='$status' WHERE id_peserta='$id'") or die(mysql_error());
        }elseif ($tdi == "") {
            $sql1 = mysql_query("select * from tb_pendaftaran WHERE id_peserta='$id'") or die (mysql_error());
            $folder1 = mysql_fetch_array($sql1);
            $tempat1 = $folder1['foto_diri'];
            $imagedelet1 = "../foto/user/$tempat1";
            unlink($imagedelet1);
            $pindah = move_uploaded_file($sumber, $target.$foto_diriup);
            $simpan = mysql_query("UPDATE tb_pendaftaran SET id_peserta='$id_peserta', no_ktp='$no_ktp', nama_peserta='$nama_peserta',  alamat='$alamat', no_hp='$no_hp', nama_perusahaan='$nama_perusahaan', produk='$produk', foto_diri='$foto_diriup', username='$username', password='$password', status='$status' WHERE id_peserta='$id'") or die(mysql_error());
        }elseif ($foto_diri == "") {
            $sql2 = mysql_query("select * from tb_pendaftaran WHERE id_peserta='$id'") or die (mysql_error());
            $folder2 = mysql_fetch_array($sql2);
            $tempat2 = $folder2['tdi'];
            $imagedelet2 = "../foto/tdi/$tempat2";
            unlink($imagedelet2);
            $pindah = move_uploaded_file($sumber2, $target2.$tdiup);
            $simpan = mysql_query("UPDATE tb_pendaftaran SET id_peserta='$id_peserta', no_ktp='$no_ktp', nama_peserta='$nama_peserta',  alamat='$alamat', no_hp='$no_hp', nama_perusahaan='$nama_perusahaan', produk='$produk', tdi='$tdiup', username='$username', password='$password', status='$status' WHERE id_peserta='$id'") or die(mysql_error());
        }else{
            $sql3 = mysql_query("select * from tb_pendaftaran WHERE id_peserta='$id'") or die (mysql_error());
            
            $folder1 = mysql_fetch_array($sql3);
            $tempat1 = $folder1['foto_diri'];
            $imagedelet1 = "../foto/user/$tempat1";
            unlink($imagedelet1);
            ...
            $simpan = mysql_query("UPDATE tb_pendaftaran SET id_peserta='$id_peserta', no_ktp='$no_ktp', nama_peserta='$nama_peserta',  alamat='$alamat', no_hp='$no_hp', nama_perusahaan='$nama_perusahaan', produk='$produk', foto_diri='$foto_diriup', tdi='$tdiup', username='$username', password='$password', status='$status' WHERE id_peserta='$id'") or die(mysql_error());
            // =============================================================
        }

              //    $sql =mysql_query("SELECT no_hp FROM tb_pendaftaran  where id_peserta='".$_POST['id_peserta']."'") or die (mysql_error());

               //     $q_pelatihan =mysql_query("SELECT nama_pelatihan FROM tb_jenispelatihan WHERE id_jenispelatihan='$id_jenispelatihan'");
                //    $dt_pelatihan = mysql_fetch_array($q_pelatihan);
                //   $nama_pelatihan = $dt_pelatihan[0];

                //    $q_jadwal =mysql_query("SELECT jadwal FROM tb_jenispelatihan WHERE id_jenispelatihan='$id_jenispelatihan");
                //    $dt_jadwal = mysql_fetch_array($q_jadwal);
                //    $jadwal = date('d F Y');$dt_jadwal[0];

                //         while ($datane = mysql_fetch_array($sql)) {
                 //        $nohp = $datane['no_hp'];
                 
                 
                //     $userkey="n6oryc"; // userkey lihat di zenziva
                //     $passkey="firman38"; // set passkey di zenziva
                //     $message="Dari seleksi persyaratan menyatakan bahwa Anda Telah ".$status." sebagai peserta magang ".$nama_magang." dengan jadwal kegiatan pelaksanaan ".$jadwal." di Dinas Tenaga Kerja Perindustian koperasi usaha kecil dan menengah Kabupaten Kudus. TERIMA KASIH";
             ...
             <ul class="nav navbar-right panel_toolbox">
              ...
              <div class="box-body">
                ...
                <!--<div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select class="form-control" name="id_jenispelatihan" value="<?php echo $data['nama_pelatihan']; ?>">
                      ...
                      <?php
                        $s_nama_pelatihan = mysql_query("SELECT * FROM tb_jenispelatihan NATURAL join tb_pelatihan") or die(mysql_error());
                        while ($h_nama_pelatihan = mysql_fetch_array($s_nama_pelatihan)) 
                        {
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select class="form-control" name="status">
                        <option value="calon" <?php if($data['status']=="calon") {echo 'selected';} ?>>Calon</option>
                ...
                <div class="form-group">
                  ...
                  <?php
                    $s_periode = mysql_query("SELECT id_periode,tahun FROM tb_periode");
