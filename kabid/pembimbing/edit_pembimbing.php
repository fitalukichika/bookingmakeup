...
if (isset($_POST['simpan'])) {
        ...
        if ($foto == "") {
             $simpan = mysql_query("UPDATE tb_pembimbing SET nik='".addslashes($nik)."', nama_pembimbing='".addslashes($nama_pembimbing)."', jekel='$jekel', alamat='$alamat', no_telp='$no_telp' WHERE id_pembimbing='$id'") or die(mysql_error());
        }else {
            $sql1 = mysql_query("select * from tb_pembimbing where id_pembimbing='$id'") or die (mysql_error());
            $folder = mysql_fetch_array($sql1);
            $tempat = $folder['foto'];
            $imagedelet = "../foto/user/$tempat";
            unlink($imagedelet);
            $pindah = move_uploaded_file($sumber, $target.$foto);
            $simpan = mysql_query("UPDATE tb_pembimbing SET nik='$nik', nama_pembimbing='$nama_pembimbing', jekel='$jekel', alamat='$alamat', no_telp='$no_telp', foto='$foto' WHERE id_pembimbing='$id'") or die(mysql_error());
        }
