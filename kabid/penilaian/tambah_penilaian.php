...
/*if (isset($_POST['simpan']) and @$_POST['penilaian'] and @$_POST['id_pemagangan'] ) 
{
  ...
  if (isset($_POST['simpan'])) {
    ...
    for ($i=0; $i < $jml; $i++) { 
    $query = mysql_query("SELECT * FROM tb_presensi A, tb_magang B, tb_magang C WHERE A.id_pemagangan=b.id_pemagangan AND B.id_magang=C.id_magang AND (A.id_peserta='".$_POST['id_peserta'][$i]."' AND C.id_magang='".$_GET['id']."')") or die(mysql_error());
    $presensi = mysql_num_rows($query);
    ...
    $simpan = mysql_query("INSERT INTO tb_penilaian VALUES (NULL, '".$_GET['id']."','".$_POST['id_peserta'][$i]."','".$_SESSION['id_user']."','".$_POST['nilai'][$i]."','$presensi','$nilai_akhir','$keterangan')") or die(mysql_error()); 
    } 
      if ($simpan) 
      {
...
if (@$_GET['id']) 
{
  $s_pembimbing = mysql_query("SELECT id_pembimbing, nama_pembimbing,tempat,jadwal,nama_magang,id_pemagangan FROM tb_pembimbing NATURAL join tb_magang natural join tb_jenismagang WHERE id_magang='".addslashes($_GET['id'])."' GROUP by id_magang") or die(mysql_error());
    ...
    <div class="row">
      <div class="col-xs-12">
        <div class="box box-info">
          ...
          <?php 
          $tgl_penilaian = array();
          $s_pre = mysql_query("SELECT date_format(tgl,'%d') as hari, date_format(tgl,'%M') as bulan, date_format(tgl,'%Y') as tahun, GROUP_concat(id_peserta) as peserta FROM tb_presensi natural join tb_magang natural join tb_jenismagang WHERE id_magang='".addslashes(@$_GET['id'])."' GROUP by tgl") or die(mysql_error());
          ...
          <div class="table-responsive" >
            ...
            <table class="table table-hover table-bordered table-striped" style="padding: 10px">
              ...
              <tbody class="tampil_daftar_peserta">
                <?php 
                $s_pembimbing = mysql_query("SELECT * FROM tb_pendaftaran natural join tb_Magang where id_magang = '".addslashes($_GET['id'])."'") or die(mysql_error());
                while ($data = mysql_fetch_array($s_pembimbing)) 
                {
                  ...
                  <tr>
                    ...
                    <td><div><input type="text" class="form-control" name="nilai[]"></div></td>
                  </tr>
                  <?php
                }
                ?>
                  <tr>
                    <td colspan="8"><center><input type="submit" class="btn btn-info" name="simpan" value="SAVE" ></center></td>
                  </tr>
              </tbody>
            </table>
            </form>
