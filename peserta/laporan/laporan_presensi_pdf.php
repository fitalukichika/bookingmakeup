...
if (@$_POST['id_magang']) 
{
	...
	class FPDF_AutoWrapTable extends FPDF {
		...
		public function rptDetailData () {
			...
			$this->Ln(30);
			$this->SetFont("", "B", 12);
			$this->SetX($left); $this->Cell(0, 10, 'Laporan Data Presensi Magang '.explode('|', $_POST['id_magang'])['1'], 0, 1,'C');
			$this->Ln(10);
	...
	$tgl_presensi = array();
	$s_pre = mysql_query("SELECT date_format(tgl,'%d') as hari, date_format(tgl,'%M') as bulan, date_format(tgl,'%Y') as tahun, GROUP_concat(id_peserta) as peserta FROM `tb_presensi` natural join tb_magang WHERE id_magang='".addslashes(@$id_magang)."' GROUP by tgl") or die(mysql_error());
	...
	$s_pembimbing = mysql_query("SELECT * FROM tb_pendaftaran natural join tb_Magang where id_magang = '".addslashes(@$id_magang)."'") or die(mysql_error());
